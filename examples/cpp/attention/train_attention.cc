...
void attention::initialize(Model& model) {
	...
	dec_lstm = LSTMBuilder(LSTM_NUM_OF_LAYERS, STATE_SIZE * 2 + EMBEDDINGS_SIZE, STATE_SIZE,
			model);
	input_lookup = model.add_lookup_parameters(VOCAB_SIZE, { EMBEDDINGS_SIZE });
	output_lookup = model.add_lookup_parameters(VOCAB_SIZE,
			{ EMBEDDINGS_SIZE });
...
vector<Expression> attention::embed_sentence(string sentence, ComputationGraph& cg){
	vector<Expression> output_exprs(sentence.size()+2);
	int index = 0;
...
vector<Expression> attention::encode_sentence(LSTMBuilder& enc_fwd_lstm, LSTMBuilder& enc_bwd_lstm, vector<Expression>& embedded){
	vector<Expression> fwd_vectors = run_lstm(enc_fwd_lstm, embedded);
	...
	vector<Expression> bwd_vectors = run_lstm(enc_bwd_lstm, embedded_rev);
	...
	for(auto loop_index =0U; loop_index < fwd_vectors.size(); loop_index++){
		encoded.push_back(concatenate({fwd_vectors.at(loop_index), bwd_vectors.at(loop_index)}));
	}
	return encoded;
}
...
Expression attention::decode(LSTMBuilder& dec_lstm, vector<Expression>& encoded, string output_sentence, ComputationGraph& cg){
	...
	for(int c : embeddings){
		Expression vector = concatenate({ attend(input_mat, dec_lstm, w1dt, cg), last_output_embeddings});
...
string attention::generate( string in_seq, LSTMBuilder& enc_fwd_lstm, LSTMBuilder& enc_bwd_lstm, LSTMBuilder& dec_lstm, ComputationGraph& cg){
	...
	for(auto loop_index =0U; loop_index < in_seq.size()*2; loop_index++){
		...
		int next_char = distance(probs.begin(),max_element(probs.begin(), probs.end()));
...
Expression attention::get_loss(string input_sentence, string output_sentence, LSTMBuilder& enc_fwd_lstm, LSTMBuilder& enc_bwd_lstm, LSTMBuilder& dec_lstm, ComputationGraph& cg){
...
void attention::train(Model& model, string sentence, SimpleSGDTrainer& trainer){
	for(int i=0; i<600; i++){
		...
		float loss_value = as_scalar(cg.forward(loss));
		cg.backward(loss);
		trainer.update();
		if(i%20 == 0){
			cout<< loss_value << endl;
			cout<<generate(sentence, enc_fwd_lstm, enc_bwd_lstm, dec_lstm, cg)<< endl;
		}
	}
}
