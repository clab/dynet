...
Expression cdiv(const Expression& x, const Expression& y) { if(y.dim().batch_size()==1)return Expression(x.pg, x.pg->add_function<ScalarQuotient>({x.i, y.i})); else return Expression(x.pg, x.pg->add_function<CwiseQuotient>({x.i, y.i}));}
