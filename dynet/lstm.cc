...
VanillaLSTMBuilder::VanillaLSTMBuilder() : has_initial_state(false), layers(0), input_dim(0), hid(0), dropout_rate_h(0), ln_lstm(false), forget_bias(true), dropout_masks_valid(false) { }

VanillaLSTMBuilder::VanillaLSTMBuilder(unsigned layers,
                                       unsigned input_dim,
                                       unsigned hidden_dim,
                                       ParameterCollection& model,
                                       bool ln_lstm, bool forget_bias) : layers(layers), input_dim(input_dim), hid(hidden_dim), ln_lstm(ln_lstm), forget_bias(forget_bias), dropout_masks_valid(false) {
  unsigned layer_input_dim = input_dim;
...
Expression VanillaLSTMBuilder::add_input_impl(int prev, const Expression& x) {
  ...
  for (unsigned i = 0; i < layers; ++i) {
    ...
    Expression i_it = logistic(i_ait);
    if (forget_bias)
        tmp2 = logistic(i_aft+ 1.f);
    else
        tmp2 = logistic(i_aft);
    Expression i_ft = tmp2;
    Expression i_ot = logistic(i_aot);
