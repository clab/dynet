...
#define DYNET_TRAINER_INST_DEV_IMPL(MyTrainer) \
  ...
  void MyTrainer::update_rule(real gscale, const std::vector<Tensor*> & values) { \
    ...
    else if(values[0]->device->type == DeviceType::GPU) { cudaSetDevice(((Device_GPU*)values[0]->device)->cuda_device_id); update_rule_dev(*(Device_GPU*)values[0]->device,gscale,values); } \
