...
struct ParameterNode : public ParameterNodeBase {
  explicit ParameterNode(ParameterIndex p) : dim(p.dim()), params(p) {}
...
struct ConstParameterNode : public Node {
  explicit ConstParameterNode(ParameterIndex p) : dim(p.dim()), params(p) {}
  std::string as_string(const std::vector<std::string>& arg_names) const override;
  Dim dim_forward(const std::vector<Dim>& xs) const override;
  void forward_impl(const std::vector<const Tensor*>& xs, Tensor& fx) const override;
  void backward_impl(const std::vector<const Tensor*>& xs,
...
struct LookupNode : public ParameterNodeBase {
  LookupNode(LookupParameterIndex p, unsigned ind) : dim(p.dim()), index(ind), pindex(&index), indices(), pindices(), params(p) {}
  LookupNode(LookupParameterIndex p, const unsigned* pind) : dim(p.dim()), index(), pindex(pind), indices(), pindices(), params(p) {}
  LookupNode(LookupParameterIndex p, const std::vector<unsigned>& indices) : dim(p.dim()), index(), pindex(), indices(indices), pindices(&this->indices), params(p) {
    dim.bd = pindices->size();
  }
  LookupNode(LookupParameterIndex p, const std::vector<unsigned>* pindices) : dim(p.dim()), index(), pindex(), indices(), pindices(pindices), params(p) {
    dim.bd = pindices->size();
  }
  std::string as_string(const std::vector<std::string>& arg_names) const override;
  Dim dim_forward(const std::vector<Dim>& xs) const override;
