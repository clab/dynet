...
namespace cnn {
    ...
    DGLSTMEMBuilder::DGLSTMEMBuilder(long layers,
        long input_dim,
        long hidden_dim,
        Model* model) : layers(layers), m_mem_dim(hidden_dim), m_align_dim(RNNEM_ALIGN_DIM)
    {
        ...
        Parameters* p_x2k0 = model->add_parameters({ long(hidden_dim), long(layer_input_dim) });
        for (unsigned i = 0; i < layers; ++i) {
            ...
            Parameters* p_x2k = model->add_parameters({ long(hidden_dim), long(layer_input_dim) });
            Parameters* p_c2k = model->add_parameters({ long(hidden_dim) });
            Parameters* p_bk = model->add_parameters({ long(hidden_dim) });
            Parameters* p_q2k = model->add_parameters({ long(hidden_dim) });

            vector<Parameters*> ps;
            if (i==0)
                ps = { p_x2i, p_h2i, p_c2i, p_bi, p_x2f, p_h2f, p_c2f, p_bf, p_x2o, p_h2o, p_c2o, p_bo, p_x2c, p_h2c, p_bc, p_wk, p_wkb, p_mem, p_interpolation, p_wa, p_wv, p_h2e, p_be, p_wo, p_ob, p_gamma, p_gamma_b, p_gamma_wtm1, p_important_b, p_woxi, p_wexi, p_bxi, p_mem_w0, p_mem_M0, p_x2k, p_c2k, p_q2k, p_bk, p_x2k0 };
            else
                ps = { p_x2i, p_h2i, p_c2i, p_bi, p_x2f, p_h2f, p_c2f, p_bf, p_x2o, p_h2o, p_c2o, p_bo, p_x2c, p_h2c, p_bc, p_wk, p_wkb, p_mem, p_interpolation, p_wa, p_wv, p_h2e, p_be, p_wo, p_ob, p_gamma, p_gamma_b, p_gamma_wtm1, p_important_b, p_woxi, p_wexi, p_bxi, p_mem_w0, p_mem_M0, p_x2k, p_c2k, p_q2k, p_bk};
    ...
    void DGLSTMEMBuilder::new_graph_impl(ComputationGraph& cg){
        param_vars.clear();
        ...
        for (unsigned i = 0; i < layers; ++i){
            ...
            if (i == 0)
            {
                ...
                vars = { i_x2i, i_h2i, i_c2i, i_bi, i_x2f, i_h2f, i_c2f, i_bf, i_x2o, i_h2o, i_c2o, i_bo, i_x2c, i_h2c, i_bc, i_wk, i_wkb, i_mem, i_interpolation, i_wa, i_wv, i_h2e, i_be, i_wo, i_ob, i_gamma, i_gamma_b, i_gamma_wtm1, i_important_b, i_woxi, i_wexi, i_bxi,
                    i_mem_w0, i_mem_M0, i_x2k, i_c2k, i_q2k, i_bk, i_x2k0
                };
            }
            else
                vars = { i_x2i, i_h2i, i_c2i, i_bi, i_x2f, i_h2f, i_c2f, i_bf, i_x2o, i_h2o, i_c2o, i_bo, i_x2c, i_h2c, i_bc, i_wk, i_wkb, i_mem, i_interpolation, i_wa, i_wv, i_h2e, i_be, i_wo, i_ob, i_gamma, i_gamma_b, i_gamma_wtm1, i_important_b, i_woxi, i_wexi, i_bxi,
                i_mem_w0, i_mem_M0, i_x2k, i_c2k, i_q2k, i_bk
            };
    ...
    void DGLSTMEMBuilder::start_new_sequence_impl(const vector<Expression>& hinit) {
        h.clear();
        c.clear();
        w.clear();
        M.clear();
    ...
    */
    std::vector<Expression> DGLSTMEMBuilder::read_memory(const int& t, const Expression & x_t, const size_t layer)
    {
    ...
    update memory 
    */
    vector<Expression> DGLSTMEMBuilder::update_memory(const int& t, const Expression & x_t, 
        const size_t layer,
    ...
    */
    Expression DGLSTMEMBuilder::compute_importance_factor(const Expression & r_t, /// retrieved memory content 
    ...
    Expression DGLSTMEMBuilder::add_input_impl(int prev, const Expression& x)
    {
        int t = prev; 
