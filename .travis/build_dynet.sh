...
if [[ "$PYTHON_INSTALL" == manual ]]; then
  ...
  if [[ "$TRAVIS_OS_NAME" == linux ]]; then
    ...
    docker run -e PYVER -e TRAVIS_OS_NAME -e CONDA_PACKAGES -e TRAVIS_BUILD_DIR=/build -v "$TRAVIS_BUILD_DIR":/build --rm "dynet-manylinux1-${BUILD_ARCH}-builder" /root/.travis/build_manylinux_wheel.sh || travis_terminate 1
  elif [[ "$TRAVIS_OS_NAME" == osx ]]; then
    .travis/build_macos_wheel.sh || travis_terminate 1
  fi
fi
